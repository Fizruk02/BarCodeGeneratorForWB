...
class code128 extends BarCode {
	...
	public function draw($im) 
	{
		...
		if($error_stop == false) 
		{
			...
			if(is_int(strpos($this->text,chr(135))) || 
				is_int(strpos($this->text,chr(136))) || 
				is_int(strpos($this->text,chr(137))) || 
				is_int(strpos($this->text,chr(138)))) 
			{
			...
			if($error_stop == false) {
				...
				for($i=0;$i<strlen($this->text);$i++) 
				{
					...
					if($this->findIndex($this->text[$i]) == 99 && 
						$this->currentCode != 'C')
					{
						$this->usingCode('C');						
					}
					elseif($this->findIndex($this->text[$i]) == 100 && 
						$this->currentCode != 'B')
					{
						$this->usingCode('B');
					}
					elseif($this->findIndex($this->text[$i]) == 101 && 
						$this->currentCode != 'A')
					{
						$this->usingCode('A');
					}
